local spider=Class("spider",Ship_base) --这个 要改“”中的名称 跟飞机名字对应就行了

function spider:initialize(side,x,y)
	Ship_base:initialize(x,y)  --x,y 不用管
	self.name="spider" --飞船的名字，
	self.quad = res.ships[side][3] --side是 blue,green,red,yellow,purple中的一种 这里不用改 数字是类型1~19*5 从上到下从左到右顺序
