local spiderIV=Class("spiderIV",Ship_base) --这个 要改“”中的名称 跟飞机名字对应就行了

function spiderIV:initialize(side,x,y)
	Ship_base:initialize(x,y)  --x,y 不用管
	self.name="spiderIV" --飞船的名字，
	self.quad = res.ships[side][60] --side是 blue,green,red,yellow,purple中的一种 这里不用改 数字是类型1~19*5 从上到下从左到右顺序
	...
	self.engineSys={
		...
		posY=3.5*self.size,
		rot=0,
		anim=res.engineFire[1]()
		},
		{posX=-8.5*self.size,
		posY=0.8*self.size,
		rot=0,
		anim=res.engineFire[3]()
		},
		{posX=-8.5*self.size,
		posY=-2*self.size,
		rot=0,
